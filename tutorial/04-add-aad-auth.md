...
Open the `./app/controllers/auth_controller.rb` file. Add a callback method to the `AuthController` class. This method will be called by the OmniAuth middleware once the OAuth flow is complete.
...
Now update the views to sign in. Open `./app/views/layouts/application.html.erb`. Replace the line `<a href="#" class="nav-link">Sign In</a>` with the following.
